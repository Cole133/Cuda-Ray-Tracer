...
class camera {
    ...
    private:
        ...
        void initialize(){
            ...
            // Calcs the horizontal and vert delta vectors
            pixel_delta_u = viewport_u / image_width;
            pixel_delta_v = viewport_v / image_height;

            // Calcs location of upper left corner
            auto viewport_upper_left = center - (focus_dist * w) - (viewport_u/2) - (viewport_v/2);
            pixel00_loc = viewport_upper_left + 0.5 * (pixel_delta_u + pixel_delta_v);

            // Calculates the cameras defoucus disk basis vectors, used to sample rays within the lens aperture.
